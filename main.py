...
def main():
    ...
    meta_path = '/data/processed/meta_All_Beauty.json.gz'
    ...
    popularity(df_path, rev_count, rating, sentiment)
    df = cbf_data('/data/processed/final.json.gz')
    idx = indices(df)
    cosim = cosine_sim(df['description'])
    cbf_ip = input('Enter product asin : ')
    cbf_recommendation(prod_asin=cbf_ip, cosine_sim = cosim, indices= idx, cbf_df = df, lim=5, min_rate=2)
